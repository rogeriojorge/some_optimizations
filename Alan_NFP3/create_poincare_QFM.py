...
import logging
logging.basicConfig()
logger = logging.getLogger('AlanQI')
logger.setLevel(1)
...
delta_R = 0.028
tmax_fl = 2000
interpolated_field_Poincare = True
tol_fieldlines = 1e-15
tol_qfm = 1e-13
nphi_QFM = 60
ntheta_QFM = 45
mpol = 14
...
OUT_DIR = os.path.join(this_path, f"output_nfp{nfp}")
os.chdir(this_path)
bs = load(os.path.join(OUT_DIR, bs_file))

pprint('Finding magnetic axis')
R_axis, Phi_axis, Z_axis = find_magnetic_axis(bs, 128, 1.0)[0]
pprint(f'  Found magnetic axis at R={R_axis}')

vmec_ran_QFM = False
if create_QFM:
    # pprint(f'Reading QFM surfaces from vmec output file wout_nfp{nfp}_Alan.nc')
    vmec = Vmec(os.path.join(this_path,f'wout_nfp{nfp}_Alan.nc'))
    s = SurfaceRZFourier.from_wout(os.path.join(this_path,f'wout_nfp{nfp}_Alan.nc'), nphi=nphi_QFM, ntheta=ntheta_QFM, range="half period")
    s.change_resolution(mpol, ntor)
    s_original_VMEC = SurfaceRZFourier.from_wout(os.path.join(this_path,f'wout_nfp{nfp}_Alan.nc'), nphi=nphi_QFM, ntheta=ntheta_QFM, range="half period")
    nfp = vmec.wout.nfp
    ...
    # s = SurfaceRZFourier(
    #     mpol=mpol, ntor=ntor, stellsym=True, nfp=nfp, quadpoints_phi=phis_QFM,
    #     quadpoints_theta=thetas_QFM)
    # ma = CurveRZFourier(nphi_QFM, 1, nfp, True)#len(vmec.wout.raxis_cc)-1, nfp, True)
    # ## Use find_magnetic_axis(bs, 64, 0.32) and Fourier transform the result to get the axis
    # ma.rc[:] = [R_axis]
    # ma.zs[:] = [Z_axis]
    ...
    os.chdir(OUT_DIR)
    vmec_QFM = Vmec(os.path.join(this_path,f'input.nfp{nfp}_Alan'))
    vmec_QFM.indata.mpol = mpol
    vmec_QFM.indata.ntor = ntor
    vmec_QFM.boundary = s
    vmec_QFM.indata.ns_array[:2]    = [   16,     51]
    vmec_QFM.indata.niter_array[:2] = [ 5000,  20000]
    vmec_QFM.indata.ftol_array[:2]  = [1e-14,  1e-14]
    vmec_QFM.write_input(os.path.join(this_path,f'input.nfp{nfp}_Alan_qfm'))
    vmec_QFM = Vmec(os.path.join(this_path,f'input.nfp{nfp}_Alan_qfm'))
    try:
        vmec_QFM.run()
        vmec_ran_QFM = True
        ...
        pprint(e)
    try:
        shutil.move(os.path.join(OUT_DIR, f"wout_nfp{nfp}_Alan_qfm_000_000000.nc"), os.path.join(this_path, f"wout_nfp{nfp}_Alan_qfm.nc"))
        os.remove(os.path.join(OUT_DIR, f'input.nfp{nfp}_Alan_qfm_000_000000'))
    except Exception as e:
        print(e)

if vmec_ran_QFM or os.path.isfile(os.path.join(this_path, f"wout_nfp{nfp}_Alan_qfm.nc")):
    vmec_QFM = Vmec(os.path.join(this_path,f'wout_nfp{nfp}_Alan_qfm.nc'))
    nfp = vmec_QFM.wout.nfp
    sys.path.insert(1, os.path.join(parent_path, '../single_stage/plotting'))
    if vmec_ran_QFM or not os.path.isfile(os.path.join(OUT_DIR, f"nfp{nfp}_Alan_qfm_VMECparams.pdf")):
        import vmecPlot2
        vmecPlot2.main(file=os.path.join(this_path, f"wout_nfp{nfp}_Alan_qfm.nc"), name=f'nfp{nfp}_Alan_qfm', figures_folder=OUT_DIR)
    nzeta=4
    ...
    if vmec_ran_QFM or not os.path.isfile(os.path.join(OUT_DIR,f"boozmn_nfp{nfp}_Alan_qfm.nc")):
        ...
        b1.run()
        b1.bx.write_boozmn(os.path.join(OUT_DIR,f"boozmn_nfp{nfp}_Alan_qfm.nc"))
        pprint("Plot BOOZ_XFORM")
        fig = plt.figure(); bx.surfplot(b1.bx, js=1,  fill=False, ncontours=35)
        plt.savefig(os.path.join(OUT_DIR, f"Boozxform_surfplot_1_nfp{nfp}_Alan_qfm.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
        fig = plt.figure(); bx.surfplot(b1.bx, js=int(boozxform_nsurfaces/2), fill=False, ncontours=35)
        plt.savefig(os.path.join(OUT_DIR, f"Boozxform_surfplot_2_nfp{nfp}_Alan_qfm.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
        fig = plt.figure(); bx.surfplot(b1.bx, js=boozxform_nsurfaces-1, fill=False, ncontours=35)
        plt.savefig(os.path.join(OUT_DIR, f"Boozxform_surfplot_3_nfp{nfp}_Alan_qfm.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
        fig = plt.figure(); bx.symplot(b1.bx, helical_detail = helical_detail, sqrts=True)
        plt.savefig(os.path.join(OUT_DIR, f"Boozxform_symplot_nfp{nfp}_Alan_qfm.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
        fig = plt.figure(); bx.modeplot(b1.bx, sqrts=True); plt.xlabel(r'$s=\psi/\psi_b$')
        plt.savefig(os.path.join(OUT_DIR, f"Boozxform_modeplot_nfp{nfp}_Alan_qfm.pdf"), bbox_inches = 'tight', pad_inches = 0); plt.close()
...
if create_Poincare:
    def trace_fieldlines(bfield, R0, Z0):
        ...
        fieldlines_tys, fieldlines_phi_hits = compute_fieldlines(
            bfield, R0, Z0, tmax=tmax_fl, tol=tol_fieldlines, comm=comm,
    ...
    if interpolated_field_Poincare:
        from simsopt.field import InterpolatedField
        from simsopt.field import SurfaceClassifier, LevelsetStoppingCriterion
        surf = SurfaceRZFourier.from_vmec_input(os.path.join(this_path,f'input.nfp{nfp}_Alan'), nphi=200, ntheta=30, range="full torus")
        nfp = surf.nfp
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, f'nfp{nfp}_Alan_poincare_QFM_fieldline_all.pdf'), bbox_inches = 'tight', pad_inches = 0)
