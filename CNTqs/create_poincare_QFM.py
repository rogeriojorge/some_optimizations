...
R_axis, Phi_axis, Z_axis = find_magnetic_axis(bs, 128, 0.28)[0]
pprint(f'  Found magnetic axis at R={R_axis}')

vmec_ran_QFM = False
if create_QFM:
    # pprint(f'Reading QFM surfaces from vmec output file wout_CNT_final.nc')
    vmec = Vmec(os.path.join(this_path,f'wout_CNT_final.nc'))
    s = SurfaceRZFourier.from_wout(os.path.join(this_path,f'wout_CNT_final.nc'), nphi=nphi_QFM, ntheta=ntheta_QFM, range="half period")
    s.change_resolution(mpol, ntor)
    s_original_VMEC = SurfaceRZFourier.from_wout(os.path.join(this_path,f'wout_CNT_final.nc'), nphi=nphi_QFM, ntheta=ntheta_QFM, range="half period")
    nfp = vmec.wout.nfp
    ...
    os.chdir(OUT_DIR)
    vmec_QFM = Vmec(os.path.join(this_path,f'input.CNT_final'))
    ...
    try:
        ...
        pprint(e)
    try:
        shutil.move(os.path.join(OUT_DIR, f"wout_CNT_qfm_000_000000.nc"), os.path.join(this_path, f"wout_CNT_qfm.nc"))
        os.remove(os.path.join(OUT_DIR, f'input.CNT_qfm_000_000000'))
    except Exception as e:
        print(e)

if vmec_ran_QFM or os.path.isfile(os.path.join(this_path, f"wout_CNT_QFM.nc")):
    vmec_QFM = Vmec(os.path.join(this_path,f'wout_CNT_QFM.nc'))
    nfp = vmec_QFM.wout.nfp
    sys.path.insert(1, os.path.join(parent_path, '../single_stage/plotting'))
    if vmec_ran_QFM or not os.path.isfile(os.path.join(OUT_DIR, "CNT_QFM_VMECparams.pdf")):
        import vmecPlot2
        vmecPlot2.main(file=os.path.join(this_path, f"wout_CNT_QFM.nc"), name='CNT_QFM', figures_folder=OUT_DIR)
    nzeta=4
    ...
    if vmec_ran_QFM or not os.path.isfile(os.path.join(OUT_DIR,"boozmn_CNT_QFM.nc")):
