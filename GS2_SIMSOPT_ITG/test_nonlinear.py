...
if args.type == 1:
    ...
    vmec_file = '/marconi_scratch/userexternal/rjorge00/some_optimizations/GS2_SIMSOPT_ITG/output_MAXITER350_least_squares_nfp2_QA_QA/wout_final.nc'
...
def create_gs2_inputs(nphi, nperiod, nlambda, nstep, dt, negrid, ngauss):
    ...
    gs2_input_name = f"gs2Input_ln{LN}lt{LT}"
    gs2_input_file = os.path.join(OUT_DIR,f'{gs2_input_name}.in')
    shutil.copy(os.path.join(this_path,'gs2Input-nl.in'),gs2_input_file)
    ...
    replace(gs2_input_file,' ny = 12',f' ny = {ny}')
    replace(gs2_input_file,' nx = 12',f' nx = {nx}')
    replace(gs2_input_file,' y0 = 10.0',f' y0 = {y0}')
...
def run_gs2(nphi, nperiod, nlambda, nstep, dt, negrid, ngauss):
    ...
    f_log = os.path.join(OUT_DIR,gs2_input_name+".log")
    with open(f_log, 'w') as fp:
        p = subprocess.Popen(f"{gs2_executable} {gs2_input_name}.in".split(),stderr=fp,stdout=fp)
    p.wait()
...
plt.savefig(os.path.join(OUT_DIR,'growthrate.png'))
plt.close()
