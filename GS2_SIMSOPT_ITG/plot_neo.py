...
hsx_neo = '/Users/rogeriojorge/local/vmec_equilibria/HSX/QHS_vac_ns201_fixed/neo_out.HSX_QHS_vacuum_ns201'
...
for i, out_dir_qaorqh in enumerate(out_dirs):
    ...
    s_radial=[]
    for x in linestoken:
        s_radial.append(float(x.split()[0])/200)
    ...
    plt.tight_layout()
    fig.savefig('neo_out_'+('QH' if i==0 else 'QA')+'.pdf', dpi=fig.dpi)#, bbox_inches = 'tight', pad_inches = 0)
    plt.close()
# plt.tight_layout()
...
for i, out_dir_qaorqh in enumerate(out_dirs):
    ...
    ax.tick_params(axis='y', labelsize=16)
    plt.tight_layout()
    plt.savefig('loss_fractions_'+('QH' if i==0 else 'QA')+'.pdf')
    plt.close()
# plt.tight_layout()
