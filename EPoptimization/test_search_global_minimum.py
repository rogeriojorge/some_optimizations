...
surf.fix("rc(0,0)")
output_to_csv = True
points_scan = np.linspace(min_bound,max_bound,npoints_scan)
...
def EPcostFunction(v:Vmec):
    ...
    try:
        ...
        print(f'Loss fraction {loss_fraction:1f} for point {(vmec.x[0]):1f}, aspect {np.abs(v.aspect()):1f} and iota {(v.mean_iota()):1f} took {(time.time()-start_time):1f}s')
        ...
        loss_fraction = 1
        print(f'Loss fraction {loss_fraction:1f} for point {(vmec.x[0]):1f} took {(time.time()-start_time):1f}s')
...
if plot_result:
    ...
    fig, ax = plt.subplots()
    plt.plot(df_scan['x(0)'], df_scan['loss_fraction'], label='Scan')
    ...
    def update(frame):
        ind_of_frame = df_opt.index[df_opt['x(0)'] == frame][0]
        ...
        return patches
    ani = FuncAnimation(fig, update, frames=df_opt['x(0)'])
    ani.save('opt_animation.gif', writer='imagemagick', fps=5)

    fig = plt.figure()
    plt.plot(df_opt['x(0)'], df_opt['loss_fraction'], 'ro', markersize=1, label='Optimizer')
    plt.plot(df_scan['x(0)'], df_scan['loss_fraction'], label='Scan')
    plt.ylabel('Loss fraction');plt.xlabel('RBC(1,0)');plt.legend();plt.savefig('loss_fraction_over_opt.pdf')
    ...
    plt.ylabel('Effective time');plt.xlabel('RBC(1,0)');plt.savefig('effective_1o_time_over_opt.pdf')