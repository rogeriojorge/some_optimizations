...
if finite_beta: vc_final = VirtualCasing.from_vmec(vmec_final, src_nphi=nphi, trgt_nphi=ntheta, trgt_ntheta=ntheta)

vmec_stage1 = Vmec(filename_stage1, mpi=mpi, verbose=True, nphi=nphi, ntheta=ntheta, range_surface='half period')
...
if finite_beta: vc_stage1 = VirtualCasing.from_vmec(vmec_stage1, src_nphi=nphi, trgt_nphi=ntheta, trgt_ntheta=ntheta)
...
if finite_beta: BdotN_surf = np.sum(Bbs * s_final.unitnormal(), axis=2) - vc_final.B_external_normal
else: BdotN_surf = np.sum(Bbs * s_final.unitnormal(), axis=2)
pointData_final = {"B·n/|B|": BdotN_surf[:, :, None]/absb_final,
             "|B|": bs_final.AbsB().reshape(s_final.gamma().shape[:2] + (1,))/meanb_final}
coilpy_plot([c.curve for c in bs_final.coils[0:ncoils]], outdir + "coils_optPlot.vtu", height=0.05, width=0.05)
...
if finite_beta: BdotN_surf = np.sum(Bbs * s_stage1.unitnormal(), axis=2) - vc_stage1.B_external_normal
else: BdotN_surf = np.sum(Bbs * s_stage1.unitnormal(), axis=2)
pointData_stage1 = {"B·n/|B|": BdotN_surf[:, :, None]/absb_stage1,
             "|B|": bs_stage1.AbsB().reshape(s_stage1.gamma().shape[:2] + (1,))/meanb_stage1}
coilpy_plot([c.curve for c in bs_stage1.coils[0:ncoils]], outdir + "coils_stage1Plot.vtu", height=0.05, width=0.05)
