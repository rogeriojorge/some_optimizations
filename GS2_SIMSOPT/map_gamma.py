...
wout_file = '/Users/rogeriojorge/local/some_optimizations/GS2_SIMSOPT/output_MAXITER350_dual_annealing_nfp4_QH/see_min/wout_nfp4_QH_000_000000.nc'
output_dir = 'out_map_nfp4_QH_dual_annealing'
s_radius = 0.25
alpha_fieldline = 0
phi_GS2 = np.linspace(-2*np.pi, 2*np.pi, 71)
nlambda = 23
LN_array = np.linspace(0.5,5,5)
LT_array = np.linspace(0.5,5,5)
...
def run_gs2(ln, lt):
    ...
    try:
        ...
        replace(gs2_input_file,' nstep = 100 ! Maximum number of timesteps',f' nstep = 250 ! Maximum number of timesteps')
    ...
    print(f'  going through LN={ln} and LT={lt} took {time()-start_time_local}s')
    return growth_rate
print('Starting GS2 scan')
start_time = time()
growth_rate_array = np.reshape(Parallel(n_jobs=n_processes_parallel)(delayed(run_gs2)(ln, lt) for ln in LN_array for lt in LT_array),(len(LN_array),len(LT_array)))
...
im = plt.imshow(growth_rate_array, interpolation='hermite', origin='lower', cmap='jet', extent=plotExtent)
