...
def output_to_csv(nzgrid, npol, nstep, dt, nhermite, nlaguerre, growth_rate, ln, lt):
    keys=np.concatenate([['ln'],['lt'],['nzgrid'],['npol'],['nlambda'],['nstep'],['nhermite'],['nlaguerre'],['dt'],['growth_rate']])
    values=np.concatenate([[ln],[lt],[nzgrid],[npol],[nlambda],[nstep],[nhermite],[nlaguerre],[dt],[growth_rate]])
...
def run_gx(nzgrid, npol, nstep, dt, nhermite, nlaguerre):
    ...
    output_to_csv(nzgrid, npol, nstep, dt, nhermite, nlaguerre, qavg, LN, LT)
    return qavg
###
### Run GS2
###
